...
class _SignInState extends State<SignIn> {
  ...
  void _signInWithEmailAndPassword() async {
    try {
      ...
      var data = await _firestore.collection("Doctors").doc(user.uid).get();
      final model = Doctor.formJson(data.data()!);
      doctor = model;
       Navigator.of(context)
            .pushNamedAndRemoveUntil('/doctorScreen', (Route<dynamic> route) => false);
      // if (data["type"] == 1) {
      //   Navigator.of(context)
      //       .pushNamedAndRemoveUntil('/HomePageDoctor', (Route<dynamic> route) => false);
      // } else {
      //   Navigator.of(context)
